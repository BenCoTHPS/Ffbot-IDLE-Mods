...
func copy_ascend_stats_to_game_dir(json_string: String):
	var dest_file = FileAccess.open("ascend_stats.json", FileAccess.WRITE)
	if dest_file:
		dest_file.store_string(json_string)
		dest_file.close()
		print("Copied ascend_stats.json to game directory")
	else:
		print("Failed to copy ascend_stats.json to game directory")

# Write ascend stats as JSON (for !ascend command)
func write_ascend_stats_json(player_name: String, playerdatabase_node, table_node, tiertracker_node, new_stats: Dictionary):
	print("Writing ascend stats JSON for: ", player_name)
	...
	var player_data = playerdatabase_node.playerdatabase[player_name]
	var current_ascension = player_data.get("ascension", 0)
	var next_ascension = current_ascension + 1
	
	# Calculate collection requirements
	var collection_current = player_data.get("collection", 0)
	var collection_total = tiertracker_node.general_pool.size() if tiertracker_node else 0
	var collection_target = round(collection_total * 0.90)
	var collection_met = collection_current >= collection_target
	
	# Calculate wins requirement
	var wins_current = player_data.get("wins", 0)
	var wins_required = 100
	var wins_met = wins_current >= wins_required
	
	# Check if player can ascend
	var can_ascend = collection_met and wins_met
	
	# Create comprehensive ascend stats JSON
	var ascend_stats_data = {
		"timestamp": Time.get_datetime_string_from_system(),
		"unix_timestamp": Time.get_unix_time_from_system(),
		"player_name": player_name,
		...
		"ascension_info": {
			...
			"exp_bonus_after_ascend": next_ascension * 2,
			"progress_reset": true
		},
		...
		"requirements": {
			"collection": {
				...
				"met": collection_met,
				"percentage": int((float(collection_current) / float(collection_target)) * 100) if collection_target > 0 else 0
			},
			"wins": {
				"current": wins_current,
				"required": wins_required,
				"met": wins_met,
				"wins_needed": max(0, wins_required - wins_current)
			}
		},
		...
		"current_stats": {
			...
			"spirit": player_data.get("spi", 0),
			"hp": player_data.get("hp", 0),
			"gil": player_data.get("gil", 0),
			"level": player_data.get("lv", 0),
			"wins": wins_current
		},
		
		"new_stats_after_ascend": {
			"attack": new_stats.get("newatk", 0),
			"magic": new_stats.get("newmag", 0),
			"spirit": new_stats.get("newspi", 0),
			"hp": new_stats.get("newhp", 0),
			"retention_percentage": 55,
			"stat_reduction": "45% loss, 55% retained"
		},
		
		"stat_changes": {
			"attack_change": new_stats.get("newatk", 0) - player_data.get("atk", 0),
			"magic_change": new_stats.get("newmag", 0) - player_data.get("mag", 0),
			"spirit_change": new_stats.get("newspi", 0) - player_data.get("spi", 0),
			"hp_change": new_stats.get("newhp", 0) - player_data.get("hp", 0)
		},
		...
		"ascension_effects": {
			...
			"exp_reset": 1,
			"wins_reset": 0,
			"party_exp_bonus": next_ascension * 2,
			"character_progression": {
				"awakening_exp_preserved": true,
				"collection_preserved": true,
				"hire_data_preserved": true,
				"mastery_preserved": true
			}
		},
		
		"status": {
			"ready_to_ascend": can_ascend,
			"awaiting_confirmation": false,
			"blocked_by_requirements": !can_ascend,
			"blocking_reasons": []
		}
	}
	
	# Add specific blocking reasons if cannot ascend
	if not can_ascend:
		if not collection_met:
			ascend_stats_data.status.blocking_reasons.append("Collection requirement not met")
		if not wins_met:
			ascend_stats_data.status.blocking_reasons.append("Wins requirement not met")
	
	var json_string = JSON.stringify(ascend_stats_data, "\t")
	print("DEBUG: Ascend stats JSON string length: ", json_string.length())
	
	# Write to user:// first
	var file = FileAccess.open("user://ascend_stats.json", FileAccess.WRITE)
	if file:
		file.store_string(json_string)
		file.close()
		print("Ascend stats JSON written to user:// for player: ", player_name)
		
		# Copy to game directory
		copy_ascend_stats_to_game_dir(json_string)
	else:
